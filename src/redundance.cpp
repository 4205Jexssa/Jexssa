...
namespace Redundance {
        ...
        pros::Optical& Optical)
    {
        ...
        // ── Motores de chasis: instalación ──────────────────────────────────
        if (!Left1.is_installed())  return 1;
        if (!Left2.is_installed())  return 2;
        if (!Left3.is_installed())  return 3;
        if (!Right1.is_installed()) return 4;
        if (!Right2.is_installed()) return 5;
        if (!Right3.is_installed()) return 6;

        // ── Motores de chasis: temperatura ──────────────────────────────────
        // if (Left1.is_over_temp()) {
        //     if (!an_temp_l1) { an_temp_l1 = true; return 7; }
        // } else { an_temp_l1 = false; }

        // if (Left2.is_over_temp()) {
        //     if (!an_temp_l2) { an_temp_l2 = true; return 8; }
        // } else { an_temp_l2 = false; }

        // if (Left3.is_over_temp()) {
        //     if (!an_temp_l3) { an_temp_l3 = true; return 9; }
        // } else { an_temp_l3 = false; }

        // if (Right1.is_over_temp()) {
        //     if (!an_temp_r1) { an_temp_r1 = true; return 10; }
        // } else { an_temp_r1 = false; }

        // if (Right2.is_over_temp()) {
        //     if (!an_temp_r2) { an_temp_r2 = true; return 11; }
        // } else { an_temp_r2 = false; }

        // if (Right3.is_over_temp()) {
        //     if (!an_temp_r3) { an_temp_r3 = true; return 12; }
        // } else { an_temp_r3 = false; }

        // // ── Motores de chasis: eficiencia (stall >= 80%) ─────────────────────
        if (Left1.get_torque() > 0.35) return 13;
        if (Left2.get_torque() > 0.35) return 14;
        if (Left3.get_torque() > 0.35) return 15;
        if (Right1.get_torque() > 0.35) return 16;
        if (Right2.get_torque() > 0.35) return 17;
        if (Right3.get_torque() > 0.35) return 18;

        // ── Motores de chasis: eficiencia (bajo rendimiento >= 60%) ─────────
        // if (Left1.get_torque() > 0.35) {
        //     if (!an_fr_l1) { an_fr_l1 = true; return 19;}
        // } else { an_fr_l1 =  false;}  // sin bandera (bug original preservado)

        // if (Left2.get_torque() > 0.35) {
        //     if (!an_fr_l2) { an_fr_l2 = true; return 20; }
        // } else { an_fr_l2 = false; }

        // if (Left3.get_torque() > 0.35) {
        //     if (!an_fr_l3) { an_fr_l3 = true; return 21; }
        // } else { an_fr_l3 = false; }

        // if (Right1.get_torque() > 0.35) {
        //     if (!an_fr_r1) { an_fr_r1 = true; return 22; }
        // } else { an_fr_r1 = false; }

        // if (Right2.get_torque() > 0.35) {
        //     if (!an_fr_r2) { an_fr_r2 = true; return 23; }
        // } else { an_fr_r2 = false; }

        // if (Right3.get_torque() > 0.35) {
        //     if (!an_fr_r3) { an_fr_r3 = true; return 24; }
        // } else { an_fr_r3 = false; }

        // // ── Motores de chasis: sobrecorriente ────────────────────────────────
        // if (Left1.is_over_current())  return 25;
        // if (Left2.is_over_current())  return 26;
        // if (Left3.is_over_current())  return 27;
        // if (Right1.is_over_current()) return 28;
        // if (Right2.is_over_current()) return 29;
        // if (Right3.is_over_current()) return 30;

        // // ── Motores de chasis: flags H-bridge ───────────────────────────────
        // if (l1f == 0x08) return 31;
        // if (l2f == 0x08) return 32;
        // if (l3f == 0x08) return 33;
        // if (r1f == 0x08) return 34;
        // if (r2f == 0x08) return 35;
        // if (r3f == 0x08) return 36;

        // if (l1f == 0x04) return 37;
        // if (l2f == 0x04) return 38;
        // if (l3f == 0x04) return 39;
        // if (r1f == 0x04) return 40;
        // if (r2f == 0x04) return 41;
        // if (r3f == 0x04) return 42;

        // ── Motores de intake/mecanismo: instalación ─────────────────────────
        if (!UMotor.is_installed()) return 43;
        if (!LMotor.is_installed()) return 44;

        // ── Motores de intake/mecanismo: temperatura ─────────────────────────
        // if (UMotor.is_over_temp()) {
        //     if (!an_temp_um) { an_temp_um = true; return 45; }
        // } else { an_temp_um = false; }

        // if (LMotor.is_over_temp()) {
        //     if (!an_temp_lm) { an_temp_lm = true; return 46; }
        // } else { an_temp_lm = false; }

        // // ── Motores de intake/mecanismo: eficiencia ──────────────────────────
        if (UMotor.get_torque() > 1) return 47;
        if (LMotor.get_torque() > 1) return 48;

        // if (UMotor.get_torque() > 0.35) {
        //     if (!an_fr_um) { an_fr_um = true; return 49; }
        // }

        // if (LMotor.get_torque() > 0.35) {
        //     if (!an_fr_lm) { an_fr_lm = true; return 50; }
        // } else { an_fr_lm = false; }

        // // ── Motores de intake/mecanismo: sobrecorriente ──────────────────────
        // if (UMotor.is_over_current()) return 51;
        // if (LMotor.is_over_current()) return 52;

        // // ── Motores de intake/mecanismo: flags H-bridge ──────────────────────
        // if (lmf == 0x08) return 53;
        // if (umf == 0x08) return 54;
        // if (lmf == 0x04) return 55;
        // if (umf == 0x04) return 56;

        // // ── IMU: velocidad excesiva ──────────────────────────────────────────
        // if (accel.x >= 1.5 || accel.x <= -1.5 ||
        //     accel.y >= 1.5 || accel.y <= -1.5 ||
        //     accel.z >= 1.5 || accel.z <= -1.5) {
        //     if (!an_ros) { an_ros = true; return 57; }
        // } else { an_ros = false; }

        // ── Controlador y radio ──────────────────────────────────────────────
        if (!Master.is_connected())  return 58;
        if (Radio.is_installed())    return 59;

        // // ── Almacenamiento USB ───────────────────────────────────────────────
        // if (!pros::usd::is_installed()) {
        //     if (!an_usd) { an_usd = true; return 60; }
        // } else { an_usd = false; }

        // // ── Batería principal ────────────────────────────────────────────────
        // if (pros::battery::get_temperature() > 85) return 61;

        // double bat_pct = (pros::battery::get_current() / pros::battery::get_capacity()) * 100;
        // if (bat_pct < 5) return 64;  // crítico primero para no saltar niveles
        // if (bat_pct < 15) return 63;
        // if (bat_pct < 30) {
        //     if (!an_bat) { an_bat = true; return 62; }
        // } else { an_bat = false; }

        // // ── Batería del controlador ──────────────────────────────────────────
        // if (Master.get_battery_level() < 15) return 66;
        // if (Master.get_battery_level() < 30) {
        //     if (!an_ctr) { an_ctr = true; return 65; }
        // } else { an_ctr = false; }

        // ── Sensores de odometría e IMU ──────────────────────────────────────
        if (!Inertial.is_installed()) {
            if (!an_cal) { an_cal = true; return 67; }
        } else { an_cal = false; }

        if (Inertial.is_calibrating()) return 77;
        if (!OdomX.is_installed())     return 78;
        if (!OdomY.is_installed())     return 79;

        // if (!Optical.is_installed()) {
        //     if (!an_opt) { an_opt = true; return 80; }
        // } else { an_opt = false; }
    ...
    void vibrate(int id, pros::Controller& Master) {
        switch (id) {
            ...
            case 4:  case 5:  case 6:   Master.rumble("- .     "); break;

            case 7:  case 8:  case 9:
            case 10: case 11: case 12:  Master.rumble("- . .   "); break;

            case 13: case 14: case 15:
            case 16: case 17: case 18:  Master.rumble("--.     "); break;

            case 19: case 20: case 21:
            case 22: case 23: case 24:  Master.rumble(". - .   "); break;

            case 25: case 26: case 27:
            case 28: case 29: case 30:  Master.rumble("-.-     "); break;

            case 31: case 32: case 33:
            case 34: case 35: case 36:  Master.rumble("-- -    "); break;

            case 37: case 38: case 39:
            case 40: case 41: case 42:  Master.rumble("---     "); break;

            case 43: case 44:           Master.rumble("-. -    "); break;
            case 45: case 46:           Master.rumble("-.. .   "); break;
            case 47: case 48:           Master.rumble("--..    "); break;
            case 49: case 50:           Master.rumble(". - ..  "); break;
            case 51: case 52:           Master.rumble("-.-     "); break;
            case 53: case 54:           Master.rumble("---.    "); break;
            case 55: case 56:           Master.rumble("----.   "); break;
            ...
            case 72: case 73: case 74: case 75: Master.rumble(". .     "); break;
    ...
    void handle_error(
        ...
        pros::Optical& Optical)
    {
        show_message(id, Left1, Left2, Left3, Right1, Right2, Right3,
                     LMotor, UMotor, Master, Radio, Inertial, OdomX, OdomY, Optical);
        pros::delay(50);
        vibrate(id, Master);
    }
    ...
    bool launch(
        ...
        pros::Optical& Optical)
    {
        int error_id = check_errors(Left1, Left2, Left3, Right1, Right2, Right3,
                                    LMotor, UMotor, Master, Radio, Inertial,
                                    OdomX, OdomY, Optical);
        if (error_id == 0) {
            Master.clear_line(0);
            lastime = 0;
        ...
        if ((pros::millis() - lastime) > 5000) {
            handle_error(error_id, Left1, Left2, Left3, Right1, Right2, Right3,
                         LMotor, UMotor, Master, Radio, Inertial, OdomX, OdomY, Optical);
            lastime = pros::millis();
            }
         return true;
